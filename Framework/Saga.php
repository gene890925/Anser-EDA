...
abstract class Saga
{
     ...
     * @return bool 是否成功
     */
    protected function isSuccess(array $info, int $retryCount = 0): bool
    {
        if (isset($info['code']) && $info['code'] == '500') {
            $this->log("收到 500 錯誤，第 " . ($retryCount + 1) . " 次重試，等待 10 秒...");
            sleep(10);
            return $this->isSuccess($info, $retryCount + 1);
        }
        return isset($info['code']) && $info['code'] == '200';
    }
}
